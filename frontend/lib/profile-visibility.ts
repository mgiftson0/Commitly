...
export type GoalVisibility = 'public' | 'followers' | 'mutuals' | 'private' | 'partners-only'

export type UserRelationship = 'owner' | 'mutual' | 'follower' | 'goal-partner' | 'none'
...
export class ProfileVisibilityManager {
  ...
  isGoalPartner(targetUserId: string): boolean {
    try {
      ...
      return goals.some((goal: any) => 
        goal.partners && goal.partners.includes(this.currentUserId) && 
        (goal.user_id === targetUserId || goal.partners.includes(targetUserId))
      )
    } catch {
      return false
    }
  ...
  canViewSection(targetUserId: string, targetProfile: UserProfile, section: keyof ProfileVisibilitySettings): boolean {
    const relationship = this.getUserRelationship(targetUserId)
    ...
    switch (relationship) {
      ...
      case 'goal-partner':
        ...
        return section === 'showGoals' ? false : targetProfile.visibility[section]
      case 'mutual':
      case 'follower':
        // Followers and mutuals see sections based on profile settings
        if (targetProfile.visibility.profileVisibility === 'private') return false
        return targetProfile.visibility[section]
      case 'none':
  ...
  canViewGoal(targetUserId: string, targetProfile: UserProfile, goal: any): boolean {
    const relationship = this.getUserRelationship(targetUserId)
    ...
    if (goal.partners && goal.partners.includes(this.currentUserId)) return true
  ...
  getFollowCounts(userId: string): { followers: number; following: number } {
    const relationships = this.getFollowRelationships()
    const followers = relationships.filter(r => r.followingId === userId).length
    const following = relationships.filter(r => r.followerId === userId).length
  ...
  filterGoalsByVisibility(goals: any[], targetUserId: string, targetProfile: UserProfile): any[] {
    return goals.filter(goal => this.canViewGoal(targetUserId, targetProfile, goal))
  }
  ...
  getVisibleSections(targetUserId: string, targetProfile: UserProfile) {
    ...
    return {
      ...
      achievements: this.canViewSection(targetUserId, targetProfile, 'showAchievements'),
      progress: this.canViewSection(targetUserId, targetProfile, 'showProgress'),
      followers: this.canViewSection(targetUserId, targetProfile, 'showFollowers'),
      following: this.canViewSection(targetUserId, targetProfile, 'showFollowing'),
      goals: this.canViewSection(targetUserId, targetProfile, 'showGoals'),
      relationship,
      isPrivateProfile: targetProfile.visibility.profileVisibility === 'private'
    }
  }
