import { createNotification } from './activity-tracker'
...
export class StreakSystem {
  ...
  checkGoalDailyRequirement(goal: any): boolean {
    ...
    if (goal.type === 'single-activity') {
      return goal.completedAt && new Date(goal.completedAt).toDateString() === today
    }
    
    if (goal.type === 'multi-activity') {
      const completedToday = goal.activities?.filter((activity: any) => 
        activity.completed && activity.completedAt && 
        new Date(activity.completedAt).toDateString() === today
      ).length || 0
      
      const threshold = Math.ceil(goal.activities.length * this.settings.multiActivityThreshold)
  ...
  checkGroupContribution(goal: any, userId: string): boolean {
    ...
    const today = new Date().toDateString()
    const userActivities = goal.activities?.filter((activity: any) => 
      activity.assigned_members?.includes(userId)
    ) || []
    
    const completedToday = userActivities.filter((activity: any) => 
      activity.completed && activity.completedAt && 
      new Date(activity.completedAt).toDateString() === today
    ).length
  ...
  calculateGroupStreak(goal: any): number {
    ...
    const membersCompleted = goal.groupMembers.filter((member: any) => {
      const memberActivities = goal.activities?.filter((activity: any) => 
        activity.assigned_members?.includes(member.id)
      ) || []
      
      return memberActivities.some((activity: any) => 
        activity.completed && activity.completedAt && 
        new Date(activity.completedAt).toDateString() === today
      )
    }).length
    ...
    const groupStreakContinues = participationRate >= this.settings.groupStreakThreshold
  ...
  calculateOverallStreak(goals: any[], userId: string): StreakData {
    ...
    const personalGoalsMet = activeGoals.filter(g => 
      !g.isGroupGoal && this.checkGoalDailyRequirement(g)
    ).length
    
    const groupGoalsMet = activeGoals.filter(g => 
      g.isGroupGoal && this.checkGroupContribution(g, userId)
    ).length
    
    const totalGoalsMet = personalGoalsMet + groupGoalsMet
    const dailyQuota = Math.ceil(activeGoals.length * this.settings.overallStreakQuota)
    ...
    const personalStreak = personalStreaks.length > 0 ? Math.max(...personalStreaks) : 0
    ...
    if (totalGoalsMet >= dailyQuota) {
      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toDateString()
      if (currentStreaks.lastUpdated === yesterday || currentStreaks.lastUpdated === today) {
        overallStreak = currentStreaks.lastUpdated === today ? overallStreak : overallStreak + 1
      } else {
  ...
  private triggerStreakNotifications(goal: any, personalStreak: number, groupStreak: number, userId: string) {
    ...
    } else if (personalStreak > previousPersonalStreak && [3, 7, 14, 30, 100].includes(personalStreak)) {
      createNotification({
...
export function calculateOverallStreak(goals: any[], userId: string = 'mock-user-id') {
  return streakSystem.calculateOverallStreak(goals, userId)
}

export function checkAtRiskStreaks(goals: any[], userId: string = 'mock-user-id') {
